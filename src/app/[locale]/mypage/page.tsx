...
export default function MyPage() {
	...
	const now = new Date();
	const certSummaries = CERTS.map((cert) => {
		...
		return {
			...
			due: allQuestions.filter((q) => isDue(srsMap[q.id], now)).length,
		};
	});

	const recentAttempts = CERTS.flatMap((cert) =>
		getAttempts(cert).map((a) => ({ ...a, cert }))
	)
