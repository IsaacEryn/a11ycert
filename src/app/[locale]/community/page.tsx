    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white no-underline hover:bg-blue-700 transition-colors"
        >
          {isKo ? "글 작성" : "New Post"}
        </Link>
      </div>
      ...
      {/* 게시글 목록 */}
      {isLoading ? (
        <p className="mt-8 text-sm text-gray-400">{isKo ? "불러오는 중..." : "Loading..."}</p>
      ) : posts.length === 0 ? (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${getCategoryBadge(post.category)}`}>
                  ...
                  <div className="mt-1 flex items-center gap-3 text-xs text-gray-400">
                    ...
                    {post.reply_count > 0 && (
                      <span>{isKo ? `댓글 ${post.reply_count}` : `${post.reply_count} replies`}</span>
                    )}
                  </div>
                </div>
              </Link>
