...
export default function RoadmapProgress({ locale, exam, domains }: Props) {
	...
	const completedCount = domains.flatMap((d) => d.units).filter((u) => isCompleted(u.id)).length;
				...
				{domains.map((domain) => (
					<section key={domain.domain} aria-labelledby={`domain-${domain.domain}`}>
						<div className="flex items-baseline gap-3">
							<h2 id={`domain-${domain.domain}`} className="text-base font-semibold text-gray-900">
