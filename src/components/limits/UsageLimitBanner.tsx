 ...
 */
export default function UsageLimitBanner({ type, remaining, limit, locale }: UsageLimitBannerProps) {
  if (!LIMITS_CONFIG.enabled) return null;
  if (remaining > 0) return null;

  const isKo = locale === "ko";
  const typeLabel = type === "quiz"
    ? (isKo ? "문제 풀이" : "quiz attempts")
    : (isKo ? "학습 페이지" : "study pages");
